...
| ☐   | **Package manager — npm _or_ pnpm** (pick one) | E1-T1         | Monorepo workspaces. TRD §3.1 allows either; decide once for consistency (see "Choices to pin"). |
...
> **Never-commit rule:** no `.env` with real values in git. Provide a committed **`.env.example`** (created in E1) listing variable _names_ only.
...
| ☐   | **Landing-page copy** — what Beyond Folio does, in marketing terms (the backlog specifies the _form_, not the words) | E2-T3         |

...
| **Broker CSV/XLSX files** | Source of all imported data — Robinhood (1 CSV), Fidelity (1 CSV), Zerodha (2 files: equity + F&O, each CSV **or** XLSX). *(Zerodha P&L/charges report not ingested in Phase 1 → INR XIRR excludes fees, OQ-I. USD fees from Fidelity commission/fees + Robinhood Gold **are** captured.)* | `BeyondFolio` (Trade, Cashflow, BrokerAccount, ImportedFile, SymbolMapping) + raw bytes to **S3** (ADR-016) | Read via an admin-managed **BrokerMapper** matched by header fingerprint; a broker format change hard-stops the import until an **admin** updates the mapper, then the user re-uploads (ADR-015) |
